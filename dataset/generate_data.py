...
def basic_tokenize(doc):
    """
    Very basic tokenizer: split the document into a list of tokens.
    """
    doc_tokens = []
    char_to_word = []
    word_to_char_start = []
...
class SQLExample(object):
    def __init__(self,
        ...
        self.valid = valid
        #如果没有指定tokens，则调用basic_tokenize将question分词然后赋值给tokens，同时计算char_to_word和word_to_char_start
        if tokens is None:
            self.tokens, self.char_to_word, self.word_to_char_start = basic_tokenize(question)
            self.value_start_end = {}
            if conditions is not None and len(conditions) > 0:
                cur_start = None
                #对于每一个condition，记录其value的起始位置和结束位置
