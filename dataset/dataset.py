...
from dataset.generate_data import SQLExample
import numpy as np
import os
import sys
...
class SQLDataset(Dataset):
    def __init__(self, data_paths, config, include_label=False):
        self.config = config
        self.tokenizer = create_tokenizer(config)
        self.input_features, self.model_inputs, self.pos = self.load_data(data_paths, config, include_label)
    ...
    def load_data(self,data_paths, config, include_label):
...
if __name__ =="__main__":
    ...
    train_data = SQLDataset(train_path,args, True)
    train_data_loader = DataLoader(train_data, batch_size=128, shuffle=True, pin_memory=True)
